...
function addGoogleBook(book){
  var currency = (book.saleInfo.country=='IL') ? book.saleInfo.country+'S' : book.saleInfo.country
...
function saveReview(bookId,review){
  var changeBook = storageService.get(BOOKS_KEY, bookId).then(book => changeBook = book).then(changeBook=> saveToStorage(changeBook,review))
  // if(changeBook.reviews) changeBook.reviews.push(review) 
  // else {
  //   changeBook.reviews = []
  //   changeBook.reviews.push(review)
  // }
  // console.log(changeBook.reviews,'/n',changeBook);
  // var change = storageService.put(BOOKS_KEY,changeBook)
  // console.log(change);
}
...
function getRandomInt(min=5, max=100) {
  min = Math.ceil(min);
  max = Math.floor(max);
